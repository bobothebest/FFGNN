...
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, node_num, edge_num, feat_dim, device,paper_features=None,
        ...
        # Add paper features processing
        if paper_features is not None:
            self.paper_features = paper_features.to(device)
            self.paper_feature_dim = paper_features.shape[1]  # Should be 768
            # Add linear transformation to match original dimension
            self.feature_transform = nn.Linear(self.paper_feature_dim, feat_dim)
        else:
            self.paper_features = None
            
        self.node_raw_embed = nn.Embedding(self.node_num, self.feat_dim)
    ...
    def get_node_features(self, node_idxs):
        """Get node features combining embeddings and paper features"""
        # Get learned embeddings
        node_embed = self.node_raw_embed(node_idxs)
        
        if self.paper_features is not None:
            # Get paper features for these nodes
            paper_feats = self.paper_features[node_idxs]
            # Transform to match embedding dimension
            transformed_feats = self.feature_transform(paper_feats)
            # Combine features (you could use other operations like concat+linear)
            node_embed = node_embed + transformed_feats
    ...
    def forward(self, cas_l, src_idx_l, target_idx_l, cut_time_l, e_l, num_neighbors=20):
        # Modify places where node_embed was used to use get_node_features
        src_node_batch_th = torch.from_numpy(src_idx_l).long().to(self.device)
        src_node_feat = self.get_node_features(src_node_batch_th)
