...
def load_paper_features(feature_path, node_dict):
    """
    Load and process paper features
    Args:
        feature_path: path to paper_features.npy
        node_dict: dictionary mapping old node ids to new ids
    Returns:
        paper_features: tensor of shape [num_nodes, 768]
    """
    # Load features
    paper_features_dict = np.load(feature_path, allow_pickle=True).item()
    
    # Initialize feature matrix
    max_idx = max(node_dict.values()) + 1
    paper_features = np.zeros((max_idx, 768))
    
    # Fill in features for nodes that have them
    for old_id, new_id in node_dict.items():
        if str(old_id) in paper_features_dict:
            paper_features[new_id] = paper_features_dict[str(old_id)]
            
    return torch.FloatTensor(paper_features)